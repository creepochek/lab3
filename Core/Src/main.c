...
int main(void)
{
  ...
  uint8_t measurements_needed = 64;
  ...
  while (1)
  {
      ...
      if (adcPoolResult == HAL_OK)
      {
          ...
          if (measurements_count < measurements_needed)
          {
              if (adcValue < 2048)
              {
          ...
          {
              averageValue = adcValueSum / measurements_needed;

              voltage = ((float)averageValue / 4096.0f) * 3.3f;
